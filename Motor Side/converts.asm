...
;	11/02/2014 Suzannah Osekowsky	wrote strings to ES instead of DS, saved address in SI
...
;													store in ES:SI instead of DS:SI
...
NegativeSign:
	MOV BYTE PTR ES:[SI],ASCIIminus		;Store ASCII minus sign in SI
...
HaveDigit:							;store digit in memory
	MOV	BYTE PTR ES:[SI],ASCIIzero	;put ASCII zero in SI
...
EndDec2StringLoop:					;Done converting
	MOV	BYTE PTR ES:[SI],00H		;add null termination
...
HaveDigitSubTen:					;store digit from 0-9 in memory
	MOV	BYTE PTR ES:[SI],ASCIIzero	;put ASCII zero in SI
	ADD ES:[SI],AX					;add digit to stored zero
	INC	SI							;increment storage location
	MOV	BX,DX						;Work with remainder
...
HaveDigitTenPlus:
	SUB	AX, 10
	MOV	BYTE PTR ES:[SI],ASCIIhexTen	;put ASCII "A" in memory location
	ADD	ES:[SI],AX						;add digit to stored ASCII A
